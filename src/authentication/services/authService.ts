...
class AuthService implements IAuthService {
  ...
  public resendEmail = async (email: string): Promise<DefaultMessageResponse> => {
    const existingUser = await User.findOne({ where: { email } });
    if (!existingUser) {
    ...
    if (existingUser.isVerified) {
      throw new Error(
        `User with email: ${existingUser.email} already verified`,
      );
    }
    ...
    try {
      ...
      return {
        message: `Thank you for registering. We have resent an email to ${email} for verification. `,
      };
    } catch (e) {
      throw new Error(
